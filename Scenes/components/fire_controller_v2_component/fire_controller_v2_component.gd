...
@export var fire_types:FIRETYPES = FIRETYPES.RANDOM
	...
	if is_active:
		...
		match fire_types:
			...
			FIRETYPES.SHOTGUN:
				...
				if shoot_cooldown > random_shoot_cooldown:
					shoot_cooldown = 0
					for barrel_marker in barrel:
						var bullet_instance = bullet_origin.instantiate() as Node2D
						get_tree().current_scene.add_child(bullet_instance)
						bullet_instance.global_position = barrel_marker.global_position
						bullet_instance.hit_box_component.damage = damage
						bullet_instance.rotation = barrel_marker.rotation
						#await get_tree().create_timer(3).timeout
						#if bullet_instance != null:
							#bullet_instance.queue_free()
